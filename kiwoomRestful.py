...
from kiwoomApi import Kiwoom, logger
...
class PriceHandler(RequestHandler):
    ...
    def post(self):
        ...
        #self.write(data)
        #code = data["lists"][0]["code"]
        #JSON Array 로 일부만 바꿔봄
        code = data["lists"][0]["code"]
        hts.dict_stock[code] = {}
...
class InfomationHandler(RequestHandler):
    def get(self):
        """
        Request Testing
        """
        uacounts = hts.kiwoom_GetAccList()
        results = hts.kiwoom_TR_OPW00001_예수금상세현황요청(uacounts[0])
        #logger.debug("예수금상세현황요청")
        #logger.debug(results)
        self.write("200 ok")


class TestPath(RequestHandler):
    def get(self):
        """
        Restful Path For TEST
        """
        #data = tornado.escape.json_decode(self.request.full_url())
        logger.debug("API TEST - Get Request")
        #logger.debug(data)
        #테스트 페이지 접속하면 가상 계좌를 보여주자
        uacounts = hts.kiwoom_GetAccList()
        logger.debug("너의 계좌는 ")
        abc = uacounts[0]
